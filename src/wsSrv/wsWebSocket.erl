...
processFrames([{Fin, Opcode, Payload} | Rest], #wsState{socket = Socket, wsMod = WsMod, webState = WebState, fragmented = Fragmented, fragmentedOpcode = FragOpcode, fragmentedBuffer = FragBuffer} = State) ->
	case Opcode of
		?WsOpCF ->
			MewState = if
				Fragmented ->
					...
					if
						Fin =:= 1 ->
							{ok, NewWebState} = doHandleMsg(FragOpcode, NewBuffer, WebState, WsMod, Socket),
		...
		?WsOpText ->
			MewState = if
				Fin =:= 1 ->
					{ok, NewWebState} = doHandleMsg(Opcode, Payload, WebState, WsMod, Socket),
					State#wsState{fragmented = false, fragmentedBuffer = <<>>, webState = NewWebState};
				true ->
		...
		?WsOpBinary ->
			...
			processFrames(Rest, MewState);
		?WsOpClose ->
			case doHandleMsg(Opcode, Payload, WebState, WsMod, Socket) of
				{ok, NewWebState} ->
...
doHandleMsg(FragOpcode, Payload, WebState, WsMod, Socket) ->
	try WsMod:handleMsg(FragOpcode, Payload, WebState) of
		{ok, NWebState} ->
			{ok, NWebState};
		...
		%% Unexpected
		Unexpected ->
			?wsErr("handleMsg return error FragOpcode:~p WebState:~p Unexpected:~p Payload:~p ~n", [FragOpcode, WebState, Unexpected, Payload]),
			{ok, WebState}
	catch
		...
		throw:Exc:Stacktrace ->
			?wsErr("handleMsg catch throw FragOpcode:~p WebState:~p Payload:~p throw:~p S:~p~n", [FragOpcode, WebState, Payload, Exc, Stacktrace]),
			{ok, WebState};
		error:Error:Stacktrace ->
			?wsErr("handleMsg catch error FragOpcode:~p WebState:~p Payload:~p Error:~p S:~p~n", [FragOpcode, WebState, Payload, Error, Stacktrace]),
			{ok, WebState};
		exit:Exit:Stacktrace ->
			?wsErr("handleMsg catch exit FragOpcode:~p WebState:~p Payload:~p Exit:~p S:~p~n", [FragOpcode, WebState, Payload, Exit, Stacktrace]),
			{ok, WebState}
	end.

sendFrame(Socket, Opcode, Payload) ->
	PayloadLen = byte_size(Payload),
	{Header, _ExtraBytes} = buildFrameHeader(Opcode, PayloadLen),
	Frame = <<Header/binary, Payload/binary>>,
	wsNet:send(Socket, Frame).

buildFrameHeader(Opcode, PayloadLen) when PayloadLen < 126 ->
	Header = <<1:1, 0:3, Opcode:4, 0:1, PayloadLen:7>>,
	{Header, 0};

buildFrameHeader(Opcode, PayloadLen) when PayloadLen =< 16#FFFF ->
	Header = <<1:1, 0:3, Opcode:4, 0:1, 126:7, PayloadLen:16>>,
	{Header, 2};

buildFrameHeader(Opcode, PayloadLen) ->
	Header = <<1:1, 0:3, Opcode:4, 0:1, 127:7, PayloadLen:64>>,
	{Header, 8}.

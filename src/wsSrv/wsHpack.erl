...
-export([new/0, new/1, encode/2, decode/2, maxTableSize/1, setMax/2]).
...
setMax(Max, Ctx = #ctx{max = Max}) ->
	Ctx;
setMax(Max, Ctx = #ctx{dyn = Dyn, size = Size}) when is_integer(Max), Max >= 0, Max =< 16#FFFFFFFF ->
	{Dyn1, Size1} = evict(Dyn, Size, Max),
	Ctx#ctx{dyn = Dyn1, size = Size1, max = Max}.
...
sizeUpdatePrefix(Ctx = #ctx{max = Max, sentMax = Max}) ->
	{[], Ctx};

...
request(wsWs, Data0, _Socket, State) ->
   Data = mergeBuffer(Data0, State),
   case wsWebSocket:parseWebSocketFrames(Data, State#wsState{buffer = <<>>}, []) of
      {ok, Frames, RemainingBuffer} ->
      ...
      {ok, {http_header, _, Key, _, Value}, Rest} ->
         ...
         case HeaderBytes > MaxHeaderSize of
            true ->
               {err_code, 431};
            false ->
               parseOneHeader(Key, Value, Rest, Socket,
                  State#wsState{headerBytes = HeaderBytes})
         end;
      {ok, http_eoh, Rest} ->
...
parseOneHeader(Key, Value, Rest, Socket,
   #wsState{headerCnt = HeaderCnt, temHeader = Headers} = State) ->
   ...
   case NewHeaderCnt > 100 of
      ...
      false ->
         NState = State#wsState{
            headerCnt = NewHeaderCnt,
            temHeader = [{Key, Value} | Headers]
         },
         case Key of
            'Content-Length' ->
...
parseContentLength(Value, Rest, Socket,
   #wsState{contentLength = Current, maxSize = MaxSize} = State) ->
...
parseTransferEncoding(Value, Rest, Socket,
   #wsState{contentLength = Current, chunkedSupp = ChunkedSupp} = State) ->
   case Current of
      N when is_integer(N) ->
         {error, content_length_transfer_encoding_conflict};
      chunked ->
      ...
      _ ->
         case validChunkedEncoding(Value) of
            ...
            true ->
               parseHeaders(Rest, Socket,
                  State#wsState{contentLength = chunked, chunkState = size})
         end
   end.
...
parseHost(Value, Rest, Socket, #wsState{wsReq = WsReq} = State) ->
   case parseHostHeader(Value) of
      {Host, Port} ->
         ...
         parseHeaders(Rest, Socket,
            State#wsState{hostHeaderSeen = true, wsReq = NewWsReq});
      error ->
         {error, invalid_host}
   end.

finishHeaders(Rest, Socket,
   #wsState{temHeader = TemHeader0, contentLength = CLen,
      wsReq = WsReq0, hostHeaderSeen = HostSeen} = State) ->
      ...
      false ->
         ...
         case CLen of
            ...
            _ ->
               ...
               NState = State#wsState{
                  stage = wsBody, buffer = <<>>, wsReq = WsReq,
...
parseFixedBody(Data, CLen,
   #wsState{bodyAcc = Acc, bodySize = Size0, wsReq = WsReq} = State) ->
   Need = CLen - Size0,
   ...
   case DataSize of
      ...
      _ ->
         ...
         {wsDone, State#wsState{
            buffer = Rest, bodyAcc = [], bodySize = CLen,
            wsReq = WsReq#wsReq{body = Body}
         }}
   end.
...
parseChunkedState(Data, #wsState{chunkState = {data, Remaining},
   bodyAcc = Acc, bodySize = Size0} = State) ->
   DataSize = byte_size(Data),
   case DataSize >= Remaining of
      ...
      true ->
         ...
         parseChunkedState(Rest, State#wsState{
            bodyAcc = [Part | Acc],
            bodySize = Size0 + Remaining,
            chunkState = crlf
         })
   end;
...
finishBody([]) -> <<>>;
finishBody(Acc) -> iolist_to_binary(lists:reverse(Acc)).
...
exceedsMax(_Length, infinity) -> false;
exceedsMax(Length, Max) -> Length > Max.

wouldExceed(_Current, _Add, infinity) -> false;
wouldExceed(Current, Add, Max) -> Current + Add > Max.

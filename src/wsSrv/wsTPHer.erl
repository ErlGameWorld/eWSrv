...
doHandle('GET', <<"/h2/info">>, #wsReq{version = Version, scheme = Scheme} = _WsReq) ->
   ...
   {ok, [{<<"Content-Type">>, <<"application/json">>}, {<<"Cache-Control">>, <<"no-store">>}], Json};
...
%% 动态大响应。按 16KB 推送，发送窗口为 0 时生产者会停住，
%% 连接进程里最多留一块，而不是整段 body。
doHandle('GET', <<"/h2/bytes/", SizeBin/binary>>, _WsReq) ->
   case boundedInt(SizeBin, 0, 64 * 1024 * 1024) of
      {ok, 0} ->
         {ok, [{<<"Content-Type">>, <<"application/octet-stream">>}, {<<"Cache-Control">>, <<"no-store">>}], <<>>};
      {ok, Size} ->
         Self = self(),
         spawn_link(fun() -> produceBytes(Self, Size) end),
         {chunk, [{<<"Content-Type">>, <<"application/octet-stream">>}, {<<"Cache-Control">>, <<"no-store">>}]};
      error ->
         {400, [], <<"size must be 0..67108864 bytes">>}
   end;

doHandle('HEAD', <<"/h2/bytes/", SizeBin/binary>>, _WsReq) ->
   case boundedInt(SizeBin, 0, 64 * 1024 * 1024) of
      {ok, _Size} ->
         {ok, [{<<"Content-Type">>, <<"application/octet-stream">>}, {<<"Cache-Control">>, <<"no-store">>}], <<>>};
      error ->
...
doHandle('GET', <<"/stream">>, _WsReq) ->
   ...
   Headers = [{<<"Content-Type">>, <<"text/plain">>}, {<<"X-Stream">>, <<"true">>}],
   {chunk, Headers};
...
doHandle('GET', <<"/file">>, _WsReq) ->
   ...
   case wsUtil:fileSize(FilePath) of
      {error, Reason} ->
      ...
      Total ->
         case parseRangeHeader(Headers) of
            {Start, undefined} when Start < Total ->
               {206, [], {file, FilePath, {Start, Total - Start}}};
            {Start, End} when Start < Total, End < Total, End >= Start ->
               {206, [], {file, FilePath, {Start, End - Start + 1}}};
            invalid_range ->
...
doHandle('PUT', <<"/resource/", Id/binary>>, WsReq) ->
   ...
   Resp = iolist_to_binary([
      <<"PUT resource ">>, Id, <<" with ">>, Size, <<" bytes\nHeaders:\n">>,
...
doHandle('HEAD', <<"/stream">>, _WsReq) ->
   {ok, [{<<"Content-Type">>, <<"text/plain">>}, {<<"X-Stream">>, <<"true">>}], <<>>};
...
schemeBin(undefined) ->
   <<"undefined">>;
schemeBin(Scheme) when is_binary(Scheme) ->
   Scheme;
schemeBin(Scheme) when is_atom(Scheme) ->
   atom_to_binary(Scheme, utf8);
schemeBin(Scheme) ->
   iolist_to_binary(io_lib:format("~p", [Scheme])).
...
%% 等连接进程确认一块已经写入发送窗口，再生成下一块。
produceBytes(Worker, Left) when Left > 0 ->
   N = erlang:min(16384, Left),
   Worker ! {chunk, binary:copy(<<$x>>, N), self()},
   receive
      {_Pid, ok} -> produceBytes(Worker, Left - N);
      {_Pid, _} -> ok
   end;
produceBytes(Worker, _) ->
   Worker ! {chunk, close, self()},
   receive
      {_Pid, _} -> ok
   end.

formatValue(Value) when is_binary(Value) ->
   Value;
formatValue(Value) when is_list(Value) ->
   list_to_binary(Value);
formatValue(Value) when is_atom(Value) ->
...
requestOrigin(Headers) ->
   case wsUtil:getHeader('Origin', Headers, undefined) of
      undefined -> wsUtil:getHeader(<<"Origin">>, Headers, <<"*">>);
      Origin -> Origin
   end.

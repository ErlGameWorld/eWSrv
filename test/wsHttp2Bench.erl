...
run(Opts) when is_map(Opts) ->
   ...
   try
      ...
      {Parser1, Tx1, NextId1, _} =
         benchBatches(Sock, Parser0, wsHpack:new(), 1, Warmup, Concurrency, Path, []),

      StartUs = erlang:monotonic_time(microsecond),
      {Parser, _Tx, _NextId, LatUs} =
         benchBatches(Sock, Parser1, Tx1, NextId1, Requests, Concurrency, Path, []),
      _ = Parser,
...
benchBatches(Sock, Parser0, Tx0, NextId0, Remaining, Concurrency, Path, LatAcc0) ->
   ...
   benchBatches(Sock, Parser, Tx, NextId, Remaining - Count, Concurrency, Path, LatAcc).
...
makeBatch(N, StreamId, Tx0, Path, FramesAcc, IdAcc) ->
   ...
   makeBatch(N - 1, StreamId + 2, Tx, Path, [Frame | FramesAcc], [StreamId | IdAcc]).

...
-export([compare/0, compare/1, matrix/0, matrix/1]).
...
compare(Opts0) when is_map(Opts0) ->
   ...
   Result = #{
      ...
      h1_over_h2_p50_latency => safeRatio(
         maps:get(latency_p50_ms, H1), maps:get(latency_p50_ms, H2)),
      h1_over_h2_p95_latency => safeRatio(
...
matrixOne(Concurrency, Base) ->
   ...
   io:format(" ~11B | ~8.0f | ~8.0f | ~5.2fx | ~9.3f | ~8.3f~n", [
      Concurrency, H1Rps, H2Rps, Ratio,
      maps:get(latency_p95_ms, H1),
      maps:get(latency_p95_ms, H2)
   ]),
   #{
      concurrency => Concurrency,
      http1 => H1,
      http2 => H2,
      h2_over_h1_throughput => Ratio
   }.

...
workerInit(Parent, Port, Path) ->
   case gen_tcp:connect({127,0,0,1}, Port,
      [binary, {packet, raw}, {active, false}, {nodelay, true}], 5000) of
      {ok, Sock} ->
...
recvBody(Sock, Status, Headers, Length, Rest, Timeout) ->
   case gen_tcp:recv(Sock, 0, Timeout) of
      {ok, Bin} ->
         recvBody(Sock, Status, Headers, Length,
            <<Rest/binary, Bin/binary>>, Timeout);
      {error, _} = Error ->
         Error
   end.

warmup(_Workers, 0) -> ok;
...
positive(N, _Name) when is_integer(N), N > 0 -> N;
positive(_N, Name) -> error({bad_option, Name}).

nonNegative(N, _Name) when is_integer(N), N >= 0 -> N;
nonNegative(_N, Name) -> error({bad_option, Name}).

lower(Bin) ->
   <<<<(lowerByte(C))>> || <<C>> <= Bin>>.

lowerByte(C) when C >= $A, C =< $Z -> C + 32;
lowerByte(C) -> C.
